...
export default function ExperimentDetail() {
  ...
  const status = experiment ? String(experiment.status) : "";
  const isRunning = status === "running";

  const fetchExperiment = (expId: string) =>
    api.experiments.get(expId).then((data) => {
      setExperiment(data);
      if (Array.isArray(data.runs)) setRuns(data.runs as unknown as RunRecord[]);
    });

  // Initial load
  useEffect(() => {
    if (!id) return;
    fetchExperiment(id).catch((e) => setError(e.message));
  }, [id]);
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {isRunning && (
            <span className="flex items-center gap-2 text-sm text-indigo-600 font-medium px-3">
